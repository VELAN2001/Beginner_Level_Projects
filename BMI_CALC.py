...
try:
    ...
    if (w==0 or h==0): 
        ...
        height=float((h/100)**2)                                                    #Converting the input of height from centimeter to meter and multiplying with the exponential 2 
        BMI=float(w/height)                                                         # formula substitution for BMI calculation
        BMI=round(BMI,2)
        print("Your BMI is \n",BMI)
        if(BMI<18.5):
